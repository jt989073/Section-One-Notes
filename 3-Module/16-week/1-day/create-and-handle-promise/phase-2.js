...
function runOnTreadmill(time) {
  ...
  return new Promise((resolve, reject) => {
  if(time < 500){
    reject('you dont have enough time to run on treadmil')
  } else {
    time -= 500
      setTimeout(() => {
...
function liftWeights(time) {
  ...
  return new Promise((resolve, reject) => {
    if(time < 2000){
      reject('you dont have enouugh time to lift weights')
    } else {
      time -= 2000
      setTimeout(() => {
...
function workout(time) {
  ...
  stretch(time)
    ...
    .then((finishTime) => console.log(`finished working out with ${finishTime / 1000} seconds left`))
