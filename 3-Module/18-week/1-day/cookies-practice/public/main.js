...
const findCookieVal = (cookieKey) => {
  const allCookies = document.cookie;
  let cookieVal = "";

  let matchedCookie = allCookies
    .split("; ")
    .find((ele) => ele.indexOf(cookieKey) === 0);
...
function storeTheme(themeName) {
  // Your code here
  document.cookie = `${THEME_NAME}=${themeName};max-age=15`;
}

// For restoring theme from cookies, if selected by the user in the past
function restoreTheme() {
  // Your code here
  const cookieVal = findCookieVal(THEME_NAME);
  if (cookieVal) {
    setTheme(cookieVal);
  }
}
...
function storeName(displayName) {
  // Your code here
  document.cookie = `${DISPLAY_NAME}=${displayName}`;
}
...
function restoreName() {
  ...
  const cookieVal = findCookieVal(DISPLAY_NAME);
  if (cookieVal) {
...
function clearName() {
  // Your code here
  document.cookie = `${DISPLAY_NAME}= ;max-age=0`;
}
