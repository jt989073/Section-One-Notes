...
// const deleteNodeBST = (rootNode, target, targetNode, newTarget, side, parent) => (parent = getParentNode(rootNode, target)) === undefined ? undefined : (targetNode = !parent ? rootNode : parent.left && parent.left.val === target ? parent.left : parent.right) && ((targetNode === rootNode || (targetNode.right && targetNode.left)) && (newTarget = inOrderPredecessor(targetNode, target)) && (deleteNodeBST(targetNode, newTarget) || true) && (targetNode.val = newTarget) ? 'ayyy lmao' : !targetNode.left && !targetNode.right && (side = parent.left === targetNode ? 'left' : 'right') && ((parent[side] = null) || true) ? 'ayyy lmao' : (!targetNode.left || !targetNode.right) && (side = parent.left === targetNode ? 'left' : 'right') && (parent[side] = targetNode.right ? (parent.left = targetNode.right) : (parent.left = targetNode.left)) ? 'ayyy lmao' : null);
