...
class PizzaProcessor:
    ...
    def pizza_production(self, employee_name=None, time_started=None, time_ended=None):
        if isinstance(employee_name, str) and isinstance(time_ended, str) and isinstance(time_started, str):
            self._pizzas_made += 1
    ...
    def pizza_pick(self, name, crust, num_toppings):
        if isinstance(name, str) and isinstance(crust, str) and isinstance(num_toppings, int) and 0 <= num_toppings <= 10:
            return f"{name.title()} ordered a {crust.lower()} crust pizza with {num_toppings} toppings!"
        else:
            return self._error
    ...
    def pizza_prepper(self, *args):
        ...
        for i,v in enumerate(args):
            if isinstance(v, int) and 0 <= v <= 10:
                if i % 2 != 0:
                    new_dict[str(i)] = {"crust": "stuffed", "toppings": v}
                else:
                    new_dict[str(i)] = {"crust": "thin", "toppings": v}
                self._pizzas_made += 1
            else:
                return self._error
        return new_dict
