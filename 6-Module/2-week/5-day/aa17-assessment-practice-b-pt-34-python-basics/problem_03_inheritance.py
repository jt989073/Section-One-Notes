...
class FlyingDragon(Dragon):
    ...
    def encounter(self):
        base_string = f"While traveling you suddenly meet a flying dragon named {self._name}! "
        if self._is_good:
            base_string += f"It lets you take a selfie and admire its {self._color} scales."
        else:
            base_string += self.breathes_fire()
        return base_string
